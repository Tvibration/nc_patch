...
public class materialAfterEdit implements IBusinessListener {
	...
	public void doAction(IBusinessEvent arg0) throws BusinessException {
		...
		if (arg0 instanceof BDCommonEvent) {
			...
			for (int i = 0; i < newObjs.length; i++) {
				if (newObjs[i] instanceof MaterialVO) {
					MaterialVO ov = (MaterialVO)newObjs[i];
					String pk_material = ov.getPk_material();
					...
					if(enablestate==1)
						...
						enablestate=2;
//					String pk_org_jb = ov.getPk_org();
//					ArrayList<String> orgs = new ArrayList<String>();
//					String sql_gss = "select s.pk_org from bd_materialstock s where s.pk_material='"+pk_material+"' and s.dr=0";
//					List<Object[]> gss = (List<Object[]>) dao.executeQuery(sql_gss, new ArrayListProcessor());
//					if(gss==null||gss.size()==0) {
//						orgs.add(pk_org_jb);
//					}else {
//						for(int k=0;k<gss.size();k++) {
//							orgs.add(String.valueOf(gss.get(k)[0]));
//						}
//					}
...
//					String defaultWarehouseSrcSystemId = (String)dao.executeQuery("select s.pk_stordoc from bd_materialstock s where s.pk_material='"+pk_material+"' and s.pk_org='"+pk_org+"'", new ColumnProcessor());
					Map map = new HashMap();
					map.put("code", code);
					map.put("name", name);
...
//					map.put("defaultWarehouseSrcSystemId",defaultWarehouseSrcSystemId);
					...
					map.put("unitHeight", ov.getUnitheight());
					map.put("unitWidth", ov.getUnitwidth());
...
//					MaterialStockVO[] msv = ov.getMaterialstock();
