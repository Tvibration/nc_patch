...
public class poOrderAfterRevise implements IBusinessListener {
		...
		if (arg0 instanceof BdUpdateEvent) {
			...
			if (newObjs instanceof OrderVO[]) {
				...
				for(int i=0;i<vos.length;i++) {
					...
					int Forderstatus = ohv.getForderstatus();
					if(Forderstatus!=3)  //订单修订才传WMS，根据采购订单状态判断，审批通过态才可以修订
						return;
					...
					for(int r=0;r<obvs.length;r++) {
						...
						map_b.put("orgSrcSystemId",obvs[r].getPk_org());
						map_b.put("inOrgSrcSystemId",obvs[r].getPk_arrvstoorg());
						mapList2.add(map_b);
					}
