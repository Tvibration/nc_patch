...
public class supplierAfterEdit implements IBusinessListener {
	...
	public void doAction(IBusinessEvent arg0) throws BusinessException {
		...
		if (arg0 instanceof BDCommonEvent) {
			...
			for (int i = 0; i < newObjs.length; i++) {
				if (newObjs[i] instanceof SupplierVO) {
					SupplierVO ov = (SupplierVO)newObjs[i];
...
//					map.put("id", id);
					...
					map.put("srcSystemId", ov.getPk_supplier());
...
//				mapList2.add(map2);
				...
				String url = WMSHttpTool.getWMSURL()+"/base/trader/ds/save-batch-by-id";
				try {
					String res = WMSHttpTool.sendPost(jsons, url);
					if(res.contains("\"success\":true")) {
						NCLocator.getInstance().lookup(ProcessLogTools.class).insertLog_RequiresNew(UFBoolean.TRUE, url, res, "", jsons, mapList.get(0).get("srcSystemId").toString(), "bd_supplier", "NC同步WMS供应商");
					}else
						throw new BusinessException(res);
				} catch (Exception e) {
					// TODO Auto-generated catch block
					NCLocator.getInstance().lookup(ProcessLogTools.class).insertLog_RequiresNew(UFBoolean.FALSE, url, "失败信息", e.getMessage(), jsons, mapList.get(0).get("srcSystemId").toString(), "bd_supplier", "NC同步WMS供应商");
