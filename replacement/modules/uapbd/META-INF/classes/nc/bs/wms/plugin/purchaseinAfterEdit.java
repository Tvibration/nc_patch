...
public class purchaseinAfterEdit implements IBusinessListener {
			...
			if (newObjs instanceof PurchaseInVO[]) {
				...
				for(int i=0;i<vos.length;i++) {
					...
					if(fpf==UFBoolean.FALSE)
						continue;
					String vdef2 = (String)dao.executeQuery("select h.vdef2 from ic_purchasein_h h where h.cgeneralhid='"+ohv.getCgeneralhid()+"'",new ColumnProcessor());
					if(vdef2!=null&&vdef2.equals("Y"))
						continue;
					PurchaseInBodyVO[] obvs = ov.getBodys();
					String pk_org = ohv.getPk_org();
					String creater = (String)dao.executeQuery("select p.code from sm_user u inner join bd_psndoc p on u.pk_psndoc=p.pk_psndoc where u.cuserid='"+ohv.getCreator()+"'", new ColumnProcessor());  //创建人
					...
					for(int r=0;r<obvs.length;r++) {
						...
						map_b.put("mainNum", obvs[r].getNshouldnum().toString());
						map_b.put("astNum", obvs[r].getNshouldassistnum().toString());
