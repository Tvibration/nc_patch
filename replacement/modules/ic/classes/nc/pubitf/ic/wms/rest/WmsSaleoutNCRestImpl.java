...
public class WmsSaleoutNCRestImpl {
	...
	public JSONString wmsDeliveryTo4C(JSONObject  str){
		...
		InvocationInfoProxy.getInstance().setGroupId("0001A1100000000016JO");  //???¨¹??????????
		...
		for (int i = 0; i < bodylists.size(); i++) {
			...
			if(!srcbids.contains(wmsbid)) {
				retjson.put("ReturnMessage", "??????["+srcvos[0].getParentVO().getVbillcode()+"]?§Ó??????§µ?["+wmsbid+"]??");
				return RestUtils.toJSONString(retjson);
			}else {
				for(DeliveryBVO srcbvo : srcbvos) {
		...
		try {
			...
			retjson.put("ReturnMessage", "??§Ö?????????????"+e1.getMessage());
			return RestUtils.toJSONString(retjson);
		}
...
//							destVo.getChildrenVO()[i].setVnotebody(clocationCode);//??¦Ë?????????
						...
						try {
							...
							retjson.put("ReturnMessage", "??????¦Ê?ID??????"+e2.getMessage());
							return RestUtils.toJSONString(retjson);
						}
...
//			destVo.getParentVO().setTaudittime(new UFDate(taudittime));
		...
		Map<String,String> rows_map = new HashMap<String,String>();  //???????????¦Ä????ID,????§Ø?????????????
...
//	 				 UFDouble ncountnum = HslParseUtil.hslMultiplyUFDouble(vchangerate, new UFDouble(ncountastnum));
				...
				retjson.put("ReturnMessage", "??????????????§µ?"+sql);
				return RestUtils.toJSONString(retjson);
			}
		}
		if(msg.length()>0) {
			retjson.put("Status", "1");
			retjson.put("ReturnMessage", "?????§Õ???:"+msg.substring(0, msg.length()-1));
			return RestUtils.toJSONString(retjson);
		}else {
			retjson.put("ReturnMessage", "?????§Õ???,??????§Õ?????????:"+msg.substring(0, msg.length()-1));
			return RestUtils.toJSONString(retjson);
		}
		...
		for (TransformBodyVO bodyvo : bodys) {
			...
			try {
				...
				if(batchcodes != null && batchcodes.length > 0){
					bodyvo.setPk_batchcode(batchcodes[0].getPk_batchcode());// ???¦Ê????????
					bodyvo.setDproducedate(batchcodes[0].getDproducedate());// ????????
					bodyvo.setDvalidate(batchcodes[0].getDvalidate());// ?§¹????
				}
			} catch (BusinessException e) {
				retjson.put("ReturnMessage", "???????¦Ê?["+bodyvo.getVbatchcode()+"]??????????????"+e);
				return RestUtils.toJSONString(retjson);
			}
...
//  	iappApi.approve(new TransformVO[]{aggvo}, null);
	...
	public JSONString WmsAddM4nBody(JSONObject str) {
		...
		String castunitid = wmsbvo.getString("castunitid");//????¦Ë
		String cunitid = wmsbvo.getString("cunitid");//????¦Ë
		String vchangerate = wmsbvo.getString("vchangerate");//??????
		String batchcode = wmsbvo.getString("batchcode");//???¦Ê?
		String sncountastnum = wmsbvo.getString("ncountastnum");//??????
		String sncountnum = wmsbvo.getString("ncountnum");//??????
		String cstateid = wmsbvo.getString("cstateid");
		...
		if(StringUtils.isNotEmpty(batchcode)){
			...
			try {
				...
				if(batchcodes != null && batchcodes.length > 0){
					newbody.setPk_batchcode(batchcodes[0].getPk_batchcode());// ???¦Ê????????
					newbody.setDproducedate(batchcodes[0].getDproducedate());// ????????
					newbody.setDvalidate(batchcodes[0].getDvalidate());// ?§¹????
					newbody.setCqualitylevelid(batchcodes[0].getCqualitylevelid());
				}
			} catch (BusinessException e) {
				retjson.put("ReturnMessage", "???????¦Ê?["+batchcode+"]??????????????"+e);
				return RestUtils.toJSONString(retjson);
			}
		}
		...
		try {
			...
			retjson.put("ReturnMessage", "????????????§Ò?????"+e);
			return RestUtils.toJSONString(retjson);
		}
		retjson.put("ReturnMessage", "¦Ä????????JSON?????"+str);
		return RestUtils.toJSONString(retjson);
	}
